...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getLocationsPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'location');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.location.count(countOptions as unknown),
