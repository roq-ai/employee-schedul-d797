...
function DashboardPage() {
  ...
  const serviceDurationStat = useSWR<PaginatedInterface<ServiceDurationInterface>>(
    () => `/service-durations`,
    () => getServiceDurations({ limit: 1, offset: 0 }),
  );
  ...
  const employeeSkillsStat = useSWR<PaginatedInterface<EmployeeSkillsInterface>>(
    () => `/employee-skills`,
    () => getEmployeeSkills({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Service Durations',
      result: serviceDurationStat?.data?.totalCount,
      isLoading: serviceDurationStat.isLoading,
    ...
    },
    {
      ...
      result: employeeSkillsStat?.data?.totalCount,
      isLoading: employeeSkillsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
